...
has fh_in => ( is => 'ro', default => sub { \*ARGV }, isa => 'GlobRef' );
...
sub _current_file {
    ...
    croak "$file_name not in ", join( " ", @{ $self->files } ),
...
sub _file_update {
    my $self = shift;
    return $self->done if not $ARGV or eof $self->{fh_in};
